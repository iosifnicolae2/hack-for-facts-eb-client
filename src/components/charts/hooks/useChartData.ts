    ...
    const analyticsInputs = useMemo(() => {
        ...
        return chart.series
            ...
            .map((series) => {
                ...
                return {
                    seriesId: series.id,
                    filter: { ...filter, years, account_category } as AnalyticsFilterType,
                };
            });
    }, [chart]);
...
export function convertToTimeSeriesData(
    ...
    dataSeriesMap.forEach((series) => {
        series.data.forEach((point) => allYears.add(Number(point.x)));
    });
    ...
    const endYear =
        chart.config.yearRange?.end ?? sortedYears[sortedYears.length - 1] ?? defaultYearRange.end;
    ...
    const data = filteredYears.map((year) => {
        ...
        dataSeriesMap.forEach((seriesData, seriesId) => {
            const yearData = seriesData.data.find((p) => Number(p.x) === year);
        ...
        if (isRelative) {
            ...
            chart.series.forEach((series) => {
                ...
                if (!firstSeriesMap.has(unit)) {
                    const v = d?.data.find((p) => Number(p.x) === year)?.y ?? 0;
            ...
            Object.keys(row).forEach((sid) => {
                ...
                if (base === 0 || !Number.isFinite(base)) {
                    warnings.push({
                        ...
                        seriesId,
                        message: `Relative base is ${base}. Auto-set ${seriesId} to 0% for year ${year}.`,
                        value: { base, year, unit: payload.initialUnit },
                    });
                    payload.value = 0;
                    payload.unit = "%";
                    ...
                    if (!Number.isFinite(computed)) {
                        warnings.push({
                            type: "auto_adjusted_value",
                            seriesId,
                            message: `Computed relative value not finite (year ${year}). Auto-set ${seriesId} to 0%.`,
                            value: { base, value: payload.value, year },
                        });
                    }
        ...
        const dataPoint: TimeSeriesDataPoint = Object.assign(row, { year });
        return dataPoint;
    });
