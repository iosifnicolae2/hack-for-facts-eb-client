# ROL

Ești un expert în Finanțe Publice din România și Analist de Date Bugetare. Sarcina ta este să generezi o fișă tehnică detaliată pentru un cod specific din clasificația economică a cheltuielilor bugetare.

# INPUT

**Codul:** 80
**Denumirea:** Împrumuturi
**Tip clasificație:** Economică (Anexa II din Ordinul MFP 1954/2005)

# INSTRUCȚIUNI DE PROCESARE

1. **Căutare și Validare:**
   - Verifică denumirea oficială în Ordinul MFP 1954/2005 (Anexa II - Clasificația economică a cheltuielilor)
   - Identifică baza legală (Legea 273/2006, Legea 500/2002)
   - Determină natura indicatorului: Operațiune financiară (+)
   - Include împrumuturi acordate de stat către diverse entități

2. **Sinteză Tehnică:**
   - Explică ce tipuri de împrumuturi acordă statul
   - Formulează logica de calcul pentru baze de date
   - Menționează că sunt operațiuni sub linia de deficit

# FORMATUL DE RĂSPUNS (OBLIGATORIU)

Te rog să structurezi răspunsul exact după următorul template, menținând un ton profesional și tehnic:

---

## Capitol 80 - Împrumuturi

### Definiție și scop

[Descriere clară a ce reprezintă acest capitol din clasificația economică.]
[Explică că sunt împrumuturi ACORDATE de stat, nu primite.]

**Baza legală principală:**

- [Act normativ 1]
- [Act normativ 2]

### Cum funcționează în practică

[Explică mecanismul: Cui acordă statul împrumuturi? Pentru ce scopuri?]

1. [Componentă - Împrumuturi pentru obiective bilaterale (80.01)]
2. [Componentă - Împrumuturi pentru refugiați (80.02)]
3. [Componentă - Împrumuturi pentru instituții publice (80.03)]
4. [Componentă - Alte împrumuturi (80.30)]

### Utilizare în calcul și impact bugetar

**Semn și Logică:**
[Explică că este operațiune financiară (+), sub linia de deficit]

**Formula simplificată:**

```
Total Împrumuturi = Împrumuturi bilaterale (80.01) + Împrumuturi refugiați (80.02) + ... + Alte împrumuturi (80.30)
```

**Impact pe tipuri de bugete:**

| Tip Buget | Tratament | Observații |
|-----------|-----------|------------|
| Buget de Stat | + | [Principal acordator] |
| Bugete Locale | + | [Împrumuturi între UAT-uri] |

### Aspecte importante

**1. [Aspect Cheie 1]:** [Diferența față de cheltuieli - sunt recuperabile]
**2. [Aspect Cheie 2]:** [Sub linia de deficit]
**3. Magnitudine:** [Estimare]

### Interpretare pentru analiză tehnică și consolidare

*Secțiune dedicată ingineriei de date și analizei financiare:*

**Pentru analiză la nivel individual:**
- [Instrucțiune clară: operațiuni financiare]

**Pentru analiză la nivel de buget general consolidat:**
- [Instrucțiune clară: împrumuturile interne se elimină]

### Întrebări frecvente (FAQ)

**Q1: Care este diferența dintre împrumuturi acordate și cheltuieli?**
A: [Răspuns concis - recuperabilitate]

**Q2: Statul acordă împrumuturi persoanelor fizice?**
A: [Răspuns concis]

**Q3: Fondul de rulment este un împrumut?**
A: [Răspuns concis]

### Documente relevante

1. [**Ordinul MFP nr. 1954/2005** - Anexa II](https://legislatie.just.ro/Public/DetaliiDocument/67596)
2. [**Legea nr. 500/2002** - Finanțele publice](https://legislatie.just.ro/Public/DetaliiDocumentAfis/37954)

### Notă importantă

[Context final despre împrumuturile acordate de stat.]

---

**REGULI STRICTE:**
1. NU inventa informații - dacă nu poți verifica, menționează explicit
2. NU include cod SQL sau sintaxă de programare
3. Toate link-urile trebuie să fie funcționale
4. Documentul trebuie să fie utilizabil de către un cetățean sau jurnalist
5. Răspunsul trebuie să fie în limba română
6. Format: Markdown

## Research URL

https://claude.ai/chat/5bb7fac6-b1a6-45ad-a59a-d665027e618b

## Processed

- **Date:** 2026-01-02 23:45:03
- **Output:** [80.md](../80.md)
