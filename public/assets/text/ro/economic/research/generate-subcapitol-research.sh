...
# =============================================================================
# Generate Subcapitol Research Files Script (Economic)
# =============================================================================
# Groups items from ec.json by subcapitol code (XX.XX), sums amounts,
...
file_exists() {
    local code="$1"
    [[ -f "$RESEARCH_DIR/research-ec-$code.md" ]] || [[ -f "$OUTPUT_DIR/${code}.md" ]]
}
...
# Main execution
echo "=========================================="
echo "Generate Subcapitol Research Files (Economic)"
echo "=========================================="
...
grouped_codes=$(jq -r '.data.aggregatedLineItems.nodes[] |
    "\(.ec_c | split(".")[0:2] | join("."))|\(.amount)"' "$EC_JSON" |
